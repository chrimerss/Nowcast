...
def make_h5(pth):
    ...
    # print(fnames)
    fnames= [fname for fname in fnames if fname[-21]=='0']
    print(fnames)
    name= pth.split('/')[-2]
    ...
    for i in range(n_splits-1):
        ...
        tmp_arr_train= np.zeros((N_FOERCAST,640,800), dtype=np.float16)
        tmp_arr_test= np.zeros((N_FOERCAST,640,800), dtype=np.float16)
        for j in range(N_FOERCAST):
            fname= sample_splits[i][j]
            arr= gdal.Open(fname).ReadAsArray()[:-9,:]
            assert arr.shape==(640, 800), 'expected shape (649,800), but got %s'%arr.shape
            tmp_arr_train[j]= arr
...
def make_h5_slide(pth):
    ...
    N_FOERCAST=10
    # sample_splits= [fnames[i*N_FOERCAST:(i+1)*N_FOERCAST] for i in range(n_splits)]
    for i in range(n_splits-1-N_FOERCAST):
        print('%d/%d'%(i,n_splits))
...
if __name__=='__main__':
    for event in ['20170826']:
        print(event)
        target_path= '/home/allen/drive/geotiff' + '/' +event+ '/'+ '*.tif'
        make_h5_slide(target_path)
        # make_h5(target_path)