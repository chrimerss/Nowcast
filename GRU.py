...
class GRUCell(nn.Module):
    def __init__(self,input_channels, hidden_channels, kernel_size, bias=False, use_gpu=True,device=None):
    ...
    def init_hidden(self, batch_size, hidden_layers,shape):
        ...
        if self.use_gpu:
            return Variable(torch.rand(batch_size,hidden_layers,shape[0],shape[1])).cuda()    #initialize variable h_0
        else:
            return Variable(torch.rand(batch_size,hidden_layers,shape[0],shape[1]))
